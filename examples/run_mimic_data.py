...
def count_inconsistencies(df: pd.DataFrame) -> int:
    ...
    if {"diagnosis_code", "hba1c_pct"}.issubset(df.columns):
        diabetic = df["diagnosis_code"].astype(str).str.upper().str.startswith(("E10", "E11"))
...
def bootstrap_stability(df: pd.DataFrame, seed: int = 99, rounds: int = 250) -> float:
    ...
    for _ in range(rounds):
        sample_idx = rng.choice(diabetic.index.to_numpy(), size=len(diabetic), replace=True)
...
def build_preliminary_outputs() -> None:
    ...
    noisy = pd.read_csv("data/mimic_cohort_raw.csv")
    ...
    pd.DataFrame(result.lineage_history).to_csv(OUTPUT_DIR / "lineage_history.csv", index=False)
...
- Real-world cohort dataset rows (MIMIC-IV extract): {len(noisy)}
